...
export default function GroupChatPage() {
  ...
  const { data: chat, isLoading } = trpc.getGroupChatMessages.useQuery(
    { chatId },
    {
  ...
  return (
    <div className="flex h-[calc(100vh-65px)] flex-col ">
      ...
      <div className="relative mx-4 mt-4 mb-2 ">
        <Input
          className="w-full rounded-xl pl-4 pr-10 h-12 "
      ...
      <div className=" bg-white px-4 py-2 m-4 rounded-xl flex flex-col h-[calc(70vh-65px)] ">
        {/* Chat Header */}
        <div className="flex items-center justify-between border-b pb-2">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="icon" onClick={() => router.back()}> <ChevronLeftIcon className="h-4 w-4" /></Button>
            <h2 className="font-semibold">{chat?.name || "Group Chat"}</h2>
          </div>
        </div>
