    ...
    <div className="p-6 mx-auto space-y-8">
      <div className="flex justify-between items-center">
        <div className="flex gap-3">
          {/* <Select value={selectedSession} onValueChange={(value) => { setSelectedSession(value); resetPage(); }} defaultValue={session?.activeSessionId}>
            ...
            <SelectContent>
              {sessions?.map((session) => (
                <SelectItem key={session.id} value={session.id}>
                  ...
                  <p className="text-xs text-gray-500">{session?.parent_session?.name}</p>
                </SelectItem>
              ))}
            </SelectContent>
          </Select> */}
