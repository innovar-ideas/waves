...
interface NewIncomeAccountFormProps {
  handleCreate: () => void;
}

export default function NewIncomeAccountForm({ handleCreate }: NewIncomeAccountFormProps) {
  ...
  const createIncomeAccount = trpc.createIncome.useMutation({
    onSuccess: () => {
      toast({
        title: "Success",
        variant: "default",
        description: "Income Account created successfully.",
      });
      form.reset();

      utils.getAllIncomeAccounts.invalidate().then(() => {
        handleCreate();
      });
    },
    onError: (error) => {
  ...
  const onSubmit = (values: AccountFormValues) => {
    createIncomeAccount.mutate(values);
  };

  return (
    <div className='mx-auto w-full space-y-6 rounded-lg bg-white p-6 shadow-lg'>
      <h4 className='font-bold'>New Income Account</h4>
      <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className='col-span-2 grid space-y-3'>
                    ...
                    <FormControl>
                      <Input data-cy='fee-name' placeholder='e.g. tuition' id='account_name' {...field} required />
                    </FormControl>
