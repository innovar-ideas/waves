...
import { columns } from "@/app/(main)/(admin)/performance-review/[id]/columns";

import { StaffPerformanceColumnType } from "./role-level-range-performance-review-column";

export default function PerformanceReviewRoleLevelPage() {
  const params = useParams();
  const id = params.id as string;
    
  const { data: performanceReview, isLoading, isError } = trpc.getPerformanceReviewRoleLevelRangeById.useQuery({
    id
  });
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            Review for role levels between {performanceReview.role_level_max} and {performanceReview.role_level_min}
          </p>
        </div>
            ...
            <CardContent className="pt-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-gray-600 dark:text-gray-400">Minimum Level:</span>
                  <Badge variant="outline" className="text-emerald-600 border-emerald-200 dark:text-emerald-400 dark:border-emerald-800">
                    Level {performanceReview.role_level_min}
                  </Badge>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-600 dark:text-gray-400">Maximum Level:</span>
                  <Badge variant="outline" className="text-emerald-600 border-emerald-200 dark:text-emerald-400 dark:border-emerald-800">
                    Level {performanceReview.role_level_max}
                  </Badge>
                </div>
              </div>
            </CardContent>
